...
onEvent('item.right_click',event =>{
  ...
  onEvent('item.right_click',event =>{
      if(event.player.mainHandItem.id == 'kubejs:jiushu_heart'){
        ...
        event.server.runCommandSilent('/improvedmobs difficulty player @p add -5')
        event.player.mainHandItem.count -= 1
        }
  })
  ...
  onEvent('item.right_click', event => {
    if ( event.player.getHeldItem(MAIN_HAND) == 'kubejs:paolude_jy'){
        if (event.player.crouching) {
            ...
            event.server.runCommandSilent(`title @p actionbar {"text":"恭喜你发现了真隐藏彩蛋","color":"green"}`)
            event.player.mainHandItem.count -= 1
           }
           else {
            event.server.runCommandSilent(`tellraw @p [{"text":"点我领取丰厚奖励","clickEvent":{"action":"open_url","value":"https://www.bilibili.com/video/BV1GJ411x7h7/"}}]`)
        }
    }
})
})