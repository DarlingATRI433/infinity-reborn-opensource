onEvent('item.registry', event =>{
  ...
  event.create('jiushu_heart').maxStackSize(64).displayName("§4救赎之心").rarity('Epic').tooltip('使用减少5点难度')
  event.create('disintegration_tablet').maxStackSize(1).rarity('Epic')
  ...
  event.create('strong_fossil').displayName("天赋强化石").tooltip('\u00A7d仅对已有天赋的装备强化').tooltip('\u00A7d很大概率失败').rarity('Epic').maxStackSize(20)

  event.create('bloody_tear').maxStackSize(1).tooltip('§4右键召唤超级血夜')
...
onEvent('item.modification', event => {
  ...
  event.modify('botania:terra_sword', modification => {
    modification.setAttackDamage(20)
